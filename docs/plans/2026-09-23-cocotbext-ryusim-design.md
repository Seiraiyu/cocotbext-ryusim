...
|-------|-------------|--------|--------|--------|
| 1 | Scaffold (`pyproject.toml`, LICENSE, namespace layout), shared smoke DUT, Makefile flow: shim, `Makefile.ryusim`, `cocotbext-ryusim-config`, `test_make_flow` including the SIM=icarus forwarding case | done | yes | yes |
| 2 | Runner flow: `RyuSim(Runner)`, registration on import, symbol-check error, `test_runner_flow` | done | yes | yes |

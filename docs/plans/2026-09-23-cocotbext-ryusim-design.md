...
|-------|-------------|--------|--------|--------|
| 1 | Scaffold (`pyproject.toml`, LICENSE, namespace layout), shared smoke DUT, Makefile flow: shim, `Makefile.ryusim`, `cocotbext-ryusim-config`, `test_make_flow` including the SIM=icarus forwarding case | pending | no | no |

...
| 4 | CI: `ci.yml` with the smoke matrix and the conformance job (§7.2) | done | yes (local) | no |
| 5 | README: install, the one-line include change, Runner and pytest usage, the explicit `--cocotb-simulator ryusim` note, the unsupported-construct list, and the cocotb version policy | done | yes | no |
